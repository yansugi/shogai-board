...
- 対象システム管理画面の「よく使う」チェックはサーバー側には保存されず、ブラウザのlocalStorageのみに記録される（不在ボードと同じ方式）。端末・ブラウザごとに個別の設定となり、他の利用者とは共有されない。チェックを付けると、障害・メンテナンス登録画面での選択候補がチップ表示され、ダッシュボードの編集・削除ボタンもチェック済みシステムのみに絞り込まれる。
