...
システム／サービス単位の障害発生状況をイントラネット上で共有するWebアプリ。要件の詳細は [requirements.md](requirements.md) を参照。
...
- **ソースコード**: [src/](src/)
- **API**: `GET /api/incidents`（現在発生中の障害一覧をJSONで返す。Slack/Teams等の連携用。詳細は[API仕様](#api仕様)を参照）
...
- **ストレージ容量の目安**: 通常デプロイの実行ファイル一式は数MB程度（別途.NET Runtimeのインストールが必要）。自己完結型デプロイの場合は`.NET Runtime`同梱で約150MB程度。DB（SQLite）はテキストデータのみで、画像等の添付は扱わないため、長期運用してもDBファイル自体は数MB〜数十MB程度に収まる見込み（不在ボードと異なり、復旧済みの障害も履歴として残す設計のため、件数次第でやや増える）。合計でも1GB程度のストレージがあれば余裕を持って運用できる。
...
- 障害の解消は、対応状況を「復旧済み」に変更することで行う（ダッシュボードからは自動的に一覧から外れる）。復旧予定時刻を過ぎても自動では復旧済みにならない（実際の復旧確認が必要なため）。
- 復旧済みの障害情報は履歴として保持され、自動削除されない（監査・振り返り用途を考慮）。ダッシュボードには表示されないが、誤登録等で不要になった情報は手動で削除できる。
